...
const getTableSchema = (TableName, partitionKey, sortKey, indexes) => {
  const KeySchema = [{
    AttributeName: partitionKey,
    KeyType:       'HASH'
  },
  ...
  /* istanbul ignore else: PAY_PER_REQUEST is available in AWS environment */
...
const createTable = async(rawClient, ...rest) => {
  const tableSchema = getTableSchema(...rest)
