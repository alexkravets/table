...
const getPrimaryKey = (TableName, partitionKey, sortKey, query, shouldThrowError = true) => {
  const partitionKeyValue = query[partitionKey]
  const sortKeyValue = query[sortKey]
  const isPrimaryKey = !!partitionKeyValue && !!sortKeyValue
...
const getConditionExpression = query => {
  ...
  for (let key in query) {
    ...
    const isLessThan    = endsWith(key, ':lt')
    const isGreaterThan = endsWith(key, ':gt')
